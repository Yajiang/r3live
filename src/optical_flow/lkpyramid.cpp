...
inline void calculate_LK_optical_flow(
    const cv::Range &range, const Mat *prevImg, const Mat *prevDeriv,
...
void opencv_LKTrackerInvoker::operator()(const cv::Range &range) const {
  calculate_LK_optical_flow(range, prevImg, prevDeriv, nextImg, prevPts,
                            nextPts, status, err, winSize, criteria, level,
                            maxLevel, flags, minEigThreshold);
}
...
inline int opencv_buildOpticalFlowPyramid(InputArray _img,
...
void LK_optical_flow_kernel::allocate_img_deriv_memory(
    std::vector<Mat> &img_pyr, std::vector<Mat> &img_pyr_deriv_I,
  ...
  for (int level = m_maxLevel; level >= 0; level--) {
    if (img_pyr_deriv_I_buff[level].cols == 0) {
      ...
      img_pyr_deriv_I_buff[level].create(
          img_pyr[level].rows + m_lk_win_size.height * 2,
...
void LK_optical_flow_kernel::calc_image_deriv_Sharr(
    std::vector<cv::Mat> &img_pyr, std::vector<cv::Mat> &img_pyr_deriv_I,
  ...
  for (int level = m_maxLevel; level >= 0; level--) {
    ...
    cv::Mat _derivI(imgSize.height + m_lk_win_size.height * 2,
                    imgSize.width + m_lk_win_size.width * 2,
                    img_pyr_deriv_I_buff[level].type(),
                    img_pyr_deriv_I_buff[level].ptr());
    img_pyr_deriv_I[level] =
        _derivI(cv::Rect(m_lk_win_size.width, m_lk_win_size.height,
                         imgSize.width, imgSize.height));
    calc_sharr_deriv(img_pyr[level], img_pyr_deriv_I[level]);
    cv::copyMakeBorder(img_pyr_deriv_I[level], _derivI, m_lk_win_size.height,
                       m_lk_win_size.height, m_lk_win_size.width,
                       m_lk_win_size.width,
...
void LK_optical_flow_kernel::set_termination_criteria(cv::TermCriteria &crit) {
  ...
  if ((m_terminate_criteria.type & TermCriteria::COUNT) == 0) {
    ...
    m_terminate_criteria.maxCount =
        std::min(std::max(m_terminate_criteria.maxCount, 0), 100);
  }
  if ((m_terminate_criteria.type & TermCriteria::EPS) == 0) {
    m_terminate_criteria.epsilon = 0.01;
  } else {
    m_terminate_criteria.epsilon =
        std::min(std::max(m_terminate_criteria.epsilon, 0.), 10.);
  }
...
void LK_optical_flow_kernel::calc(InputArray _prevImg, InputArray _nextImg,
  ...
  CV_Assert(m_maxLevel >= 0 && m_lk_win_size.width > 2 &&
            m_lk_win_size.height > 2);
  ...
  m_maxLevel = opencv_buildOpticalFlowPyramid(_prevImg, m_prev_img_pyr,
                                              m_lk_win_size, m_maxLevel, false);
  m_maxLevel = opencv_buildOpticalFlowPyramid(_nextImg, m_curr_img_pyr,
                                              m_lk_win_size, m_maxLevel, false);
  calc_image_deriv_Sharr(m_prev_img_pyr, m_prev_img_deriv_I,
                         m_prev_img_deriv_I_buff);

  for (level = m_maxLevel; level >= 0; level--) {
    // cout << "Image size = " << prevPyr[level * lvlStep1].size() << ", level =
    // " << level << endl;
    ...
    parallel_for_(Range(0, n_points),
                  opencv_LKTrackerInvoker(
                      &m_prev_img_pyr[level], &m_prev_img_deriv_I[level],
                      &m_curr_img_pyr[level], prevPts, nextPts, status, err,
...
int LK_optical_flow_kernel::track_image(
    const cv::Mat &curr_img, const std::vector<cv::Point2f> &last_tracked_pts,
    std::vector<cv::Point2f> &curr_tracked_pts, std::vector<uchar> &status,
    int opm_method) {
  // Common_tools::Timer tim;
  // tim.tic();
  // printf_line;
  m_maxLevel = opencv_buildOpticalFlowPyramid(curr_img, m_curr_img_pyr,
                                              m_lk_win_size, m_maxLevel, false);
  calc_image_deriv_Sharr(m_curr_img_pyr, m_curr_img_deriv_I,
                         m_curr_img_deriv_I_buff);
  if (m_prev_img_pyr.size() == 0 ||
      (m_prev_img_pyr[0].cols == 0)) // The first img
  {
    m_prev_img_pyr.resize(m_curr_img_pyr.size());
    allocate_img_deriv_memory(m_curr_img_pyr, m_prev_img_deriv_I,
                              m_prev_img_deriv_I_buff);
    swap_image_buffer();
...
void calculate_optical_flow(InputArray _prevImg, InputArray _nextImg,
                            ...
                            OutputArray _status, OutputArray _err, Size winSize,
                            int maxLevel, TermCriteria criteria, int flags,
                            double minEigThreshold) {
  // printf_line;
  cv::Ptr<LK_optical_flow_kernel> optflow_kernel =
      cv::makePtr<LK_optical_flow_kernel>(winSize, maxLevel, criteria, flags,
                                          minEigThreshold);
