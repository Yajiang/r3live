...
void FOVChecker::SetEnv(BoxPointType env_param){
    env = env_param;
}

void FOVChecker::SetBoxLength(double box_len_param){
    box_length = box_len_param;
}

void Round3D(Eigen::Vector3d &vec, int decimal){
    double tmp;
    int t;
    for (int i = 0; i < 3; i++){
        t = pow(10,decimal);
...
void FOVChecker::CheckFov(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, vector<BoxPointType> &boxes){
    Round3D(cur_pose,4);
    Round3D(axis,3);
    axis = axis/axis.norm();
    ...
    for (i=start_i; i<=maxn; i++){
        ...
        for (j = 1; j <= max_vN; j++){
            ...
            while (k>=0){
                ...
                if (!CheckBox(cur_pose, axis, theta, depth, box)) u_min = u_min + pow(2,k);
                k = k-1;
            }
            k = max_ulogN;
            u_max = 0;
            while (k>=0){
                ...
                if (!CheckBox(cur_pose, axis, theta, depth, box)) u_max = u_max + pow(2,k);
        ...
        for (j = 1; j <= max_vN; j++){
            ...
            while (k>=0){
                ...
                box.vertex_max[1] = box_p_max(1); 
                box.vertex_max[2] = box_p_max(2); 
                if (!CheckBox(cur_pose, axis, theta, depth, box)) {
                    u_max = u_max + pow(2,k);
...
bool FOVChecker::CheckBox(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, const BoxPointType box){
    ...
    for (int i = 0; i < 8; i++){
        if (CheckPoint(cur_pose, axis, theta, depth, vertex[i])){
            return true;
        } 
    }
    Eigen::Vector3d center_point = (vertex[7]+vertex[0])/2.0;
    if (CheckPoint(cur_pose, axis, theta, depth, center_point)){
        return true;
    }
    PlaneType plane[6];
    ...
    if (CheckSurface(cur_pose, axis, theta, depth, plane[0]) || CheckSurface(cur_pose, axis, theta, depth, plane[1]) || CheckSurface(cur_pose, axis, theta, depth, plane[2]) || CheckSurface(cur_pose, axis, theta, depth, plane[3]) || CheckSurface(cur_pose, axis, theta, depth, plane[4]) || CheckSurface(cur_pose, axis, theta, depth, plane[5]))
        s = 1;
    else
        s = 0;
    return s;
...
bool FOVChecker::CheckSurface(Eigen::Vector3d cur_pose, Eigen::Vector3d axis,  double theta, double depth, PlaneType plane){
    ...
    if (CheckLine(cur_pose, axis, theta, depth, plane_p, plane_u) || CheckLine(cur_pose, axis, theta, depth, plane_p, plane_v) || CheckLine(cur_pose, axis, theta, depth, plane_p + plane_u, plane_v) || CheckLine(cur_pose, axis, theta, depth, plane_p + plane_v, plane_u)){
        s = 1;
        return s;
    }
    pc = plane_p + (plane.p[3]-plane.p[0])/2;
    if (CheckPoint(cur_pose, axis, theta, depth, pc)){
        s = 1;
        return s;
    }
...
bool FOVChecker::CheckLine(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, Eigen::Vector3d line_p, Eigen::Vector3d line_vec){
    ...
    if (!(fabs(A)<=eps_value)){
        ...
        if (delta <= eps_value){
            ...
            if ((t1>=-eps_value && t1<=1+eps_value) && CheckPoint(cur_pose, axis, theta, depth, line_p + line_vec * t1)){
                s = true;
                return s;
            }
            // printf("%d\n",check_point(cur_pose, axis, theta, depth, line_p + line_vec * t2));
            if ((t2>=-eps_value && t2<=1+eps_value) && CheckPoint(cur_pose, axis, theta, depth, line_p + line_vec * t2)){
                s = true;
                return s;
            }
...
bool FOVChecker::CheckPoint(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, Eigen::Vector3d point){
    Eigen::Vector3d vec;
    double proj_len;
    bool s;
    vec = point-cur_pose;
...
bool FOVChecker::CheckBoxInEnv(BoxPointType box){
