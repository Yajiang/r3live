...
void IMUFusion::imuCallback(const sensor_msgs::Imu::ConstPtr &msg_in) {
  ...
  RPA_DEBUG("imu data: ", msg->linear_acceleration.x);

  // std::cout<<"imu data: "<< msg->linear_acceleration.x << " "
  // << msg->linear_acceleration.y << " "
  // << msg->linear_acceleration.z << " \n"
  // << std::endl;
  imuBufferLio.emplace_back(msg);
  imuBufferVio.emplace_back(msg);
...
void IMUFusion::printFieldName(sensor_msgs::PointCloud2::ConstPtr &msg) {
...
auto IMUFusion::getPointcloudFromROS(
    sensor_msgs::PointCloud2::ConstPtr &msg,
    pcl::PointCloud<pcl::PointXYZINormal> &pcl_pc) -> bool {
...
auto IMUFusion::syncPackages(MeasureGroup &meas) -> bool {
  if (lidar_buffer.empty() || imuBufferLio.empty()) {
    return false;
  }
  ...
  if (lastImuTimestamp < lidar_end_time) {
    return false;
  }

  /*** push imu data, and pop from imu buffer ***/
  double imu_time = imuBufferLio.front()->header.stamp.toSec();
  meas.imu.clear();
  while ((!imuBufferLio.empty()) && (imu_time < lidar_end_time)) {
    imu_time = imuBufferLio.front()->header.stamp.toSec();
    if (imu_time > lidar_end_time + 0.02)
      break;
    meas.imu.push_back(imuBufferLio.front());
    imuBufferLio.pop_front();
  }
...
void IMUFusion::pointBodyToWorld(PointType const *const pi,
                                 PointType *const po) {
...
void IMUFusion::colorPointBodyToWorld(PointType const *const ptIn,
                                    pcl::PointXYZI *const ptOut) {
  Eigen::Vector3d ptBody(ptIn->x, ptIn->y, ptIn->z);
  Eigen::Vector3d ptGlobal(g_lio_state.rot_end *
                               (ptBody + Lidar_offset_to_IMU) +
                           g_lio_state.pos_end);
  ...
  float intensity = ptIn->intensity;
  intensity = intensity - std::floor(intensity);

  int reflection_map = intensity * 10000;
}

int IMUFusion::get_cube_index(const int &i, const int &j, const int &k) {
  return (i + laserCloudWidth * j + laserCloudWidth * laserCloudHeight * k);
}

bool IMUFusion::center_in_FOV(Eigen::Vector3f cube_p) {
...
bool IMUFusion::if_corner_in_FOV(Eigen::Vector3f cube_p) {
  Eigen::Vector3f dis_vec = g_lio_state.pos_end.cast<float>() - cube_p;
  float squaredSide1 = dis_vec.transpose() * dis_vec;
  dis_vec = XAxisPoint_world.cast<float>() - cube_p;
  float squaredSide2 = dis_vec.transpose() * dis_vec;
...
void IMUFusion::lasermap_fov_segment() {
...
void IMUFusion::featurePointsCallback(
...
void IMUFusion::wait_render_thread_finish() {
...
auto IMUFusion::serviceLIOUpdate() -> int {
  nav_msgs::Path path;
  ...
  for (auto & i : featsArray) {
    i.reset(new PointCloudXYZINormal());
  }
  ...
  setInitialStateCov(g_lio_state);
  while (ros::ok()) {
    if (flg_exit)
      break;
    ros::spinOnce();
    ...
    if (1) {
      ...
      if ((feats_down->points.size() > 1) && (ikdtree.Root_Node == nullptr)) {
        ...
        std::cout << "Initialize Map iKD-Tree size: ~~~~~~~"
                  << feats_down->points.size() << std::endl;
      ...
      int featsFromMapNum = ikdtree.size();
      {
        ...
        // ); featsFromMap->clear(); featsFromMap->points = ikdtree.PCL_Storage;
        ...
        //     icp.getFitnessScore() << std::endl; std::cout << "Transformation
        //     matrix:\n"
      ...
      if (featsFromMapNum >= 5) {
        ...
        for (iterCount = 0; iterCount < NUM_MAX_ITERATIONS; iterCount++) {
          ...
          // std::cout <<
          // "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          // :" << std::endl; std::cout << "state before optimize:" <<
          // std::endl; std::cout <<
          // "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          // :" << std::endl;
          CheckAbnormalState(g_lio_state);
          tim.tic("Iter");
          ...
          // printf_line;
          g_lio_state.lastUpdateTime = Measures.lidar_end_time;
              ...
              (iterCount == NUM_MAX_ITERATIONS - 1)) // Fast lio ori version.
          {
            if (flg_EKF_inited) {
              ...
              // {
              //     g_lio_state.vel_end( idx ) =
              //     sigmoid_penalty(g_lio_state.vel_end( idx ),100);
              // }
      ...
      *laserCloudFullRes2 = dense_map_en ? (*feats_undistort) : (*feats_down);
      ...
      laserCloudFullResColor->clear();
      {
        for (int i = 0; i < laserCloudFullResNum; i++) {
          colorPointBodyToWorld(&laserCloudFullRes2->points[i], &temp_point);
          ...
          (g_LiDAR_frame_index < 100)) // append point cloud to global map.
      {
        ...
        if (m_if_record_mvs) {
          std::vector<std::shared_ptr<RGBPoints>> pts_last_hitted;
          pts_last_hitted.reserve(1e6);
          m_number_of_new_visited_voxel =
              m_mapColorPoints.append_points_to_global_map(
                  *laserCloudFullResColor,
                  Measures.lidar_end_time -
                      g_camera_lidar_queue.m_first_imu_time,
                  &pts_last_hitted, m_append_global_map_point_step);
          m_mapColorPoints.m_mutex_pts_last_visited->lock();
          m_mapColorPoints.m_pts_last_hitted = pts_last_hitted;
          m_mapColorPoints.m_mutex_pts_last_visited->unlock();
        } else {
          m_number_of_new_visited_voxel =
              m_mapColorPoints.append_points_to_global_map(
                  *laserCloudFullResColor,
      ...
      if (1) // Uncomment this code scope to enable the publish of effective
             // points.
      {
        ...
        pcl::PointXYZI temp_point;
        for (int i = 0; i < laserCloudSelNum; i++) {
          colorPointBodyToWorld(&laserCloudOri->points[i], &temp_point);
          laserCloudFullResColor->push_back(temp_point);
        }
        sensor_msgs::PointCloud2 laserCloudFullRes3;
        pcl::toROSMsg(*laserCloudFullResColor, laserCloudFullRes3);
        std::cout << "effective points size :" << laserCloudFullResColor->size()
                  << std::endl;
        // laserCloudFullRes3.header.stamp = ros::Time::now();
        // //.fromSec(last_timestamp_lidar);
