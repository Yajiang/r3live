...
void IMUFusion::pointBodyToWorld( PointType const *const pi, PointType *const po )
{
...
void IMUFusion::RGBpointBodyToWorld( PointType const *const pi, pcl::PointXYZI *const po )
{
    Eigen::Vector3d p_body( pi->x, pi->y, pi->z );
    Eigen::Vector3d p_global( g_lio_state.rot_end * ( p_body + Lidar_offset_to_IMU ) + g_lio_state.pos_end );
...
void IMUFusion::feat_points_cbk( const sensor_msgs::PointCloud2::ConstPtr &msg_in )
{
...
int IMUFusion::service_LIO_update()
{
    nav_msgs::Path path;
