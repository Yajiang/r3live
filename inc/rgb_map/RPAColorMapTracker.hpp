  ...
  public:
    ...
    void update_last_tracking_vector_and_ids()
    {
        ...
        for ( auto it = m_map_rgb_pts_in_last_frame_pos.begin(); it != m_map_rgb_pts_in_last_frame_pos.end(); it++ )
        {
            ...
            m_colors.push_back( ( ( RGBPoints * ) it->first )->m_dbg_color );
            m_old_ids.push_back( idx );
            idx++;
        }
    }

    void set_track_pts( cv::Mat &img, std::vector< std::shared_ptr< RGBPoints > > &rgb_pts_vec, std::vector< cv::Point2f > &pts_proj_img_vec )
    {
    ...
    void init( const std::shared_ptr< ImageFrame > &img_with_pose, std::vector< std::shared_ptr< RGBPoints > > &rgb_pts_vec, std::vector< cv::Point2f > &pts_proj_img_vec )
    {
    ...
    void update_and_append_track_pts( std::shared_ptr< ImageFrame > &img_pose, GlobalMap &map_rgb, double mini_dis = 10.0, int minimum_frame_diff = 3e8 );
    void reject_error_tracking_pts( std::shared_ptr< ImageFrame > &img_pose, double dis = 2.0 );
    ...
    void track_img( std::shared_ptr< ImageFrame > &img_pose, double dis = 2.0, int if_use_opencv = 1 );
    int get_all_tracked_pts( std::vector< std::vector< cv::Point2f > > *img_pt_vec = nullptr );
    int remove_outlier_using_ransac_pnp( std::shared_ptr< ImageFrame > &img_pose, int if_remove_ourlier = 1 );
};
